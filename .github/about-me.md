...
generation. Agents support multiple tech stacks — Java/Spring Boot, Node.js, Python, Go, and more — via
per-repo local config generated by the `repo-config` agent.
...
├── agents/
│   ├── repo-config.agent.md                          # Repo config generator agent ← NEW
│   ├── repo-config.agent.md.template                 # Template (for installer)
...
├── config/
│   ├── copilot-config.yml                            # This repo's committed config (source repo only)
│   ├── copilot-config.template.yml                   # Global installer template (Java/Spring Boot default)
│   ├── copilot-config.java-spring.template.yml       # Stack template: Java/Spring Boot ← NEW
│   ├── copilot-config.nodejs.template.yml            # Stack template: Node.js/TypeScript ← NEW
│   ├── copilot-config.python.template.yml            # Stack template: Python ← NEW
│   └── copilot-config.generic.template.yml           # Stack template: Generic ← NEW
├── docs/
...
└── skills/
    ├── backward-compat/
    │   ├── backward-compat.skill.md          # Backward compatibility audit skill ← NEW
    │   └── backward-compat.skill.md.template # Skill template (for installer) ← NEW
    ├── code-review/
...
> **Note:** In developer repos (not this source repo), `.github/copilot-config.yml` is **gitignored** and
> generated by the `repo-config` agent. Each developer runs it once per repo.

## 🎯 Purpose

This GitHub Copilot configuration provides:

1. **Repo Config Generation** - `repo-config` agent auto-detects stack and generates per-repo config (gitignored)
2. **TDD Test Generation** - Generate failing unit tests (RED phase)
3. **Code Review** - Automated code reviews with SOLID, DRY, KISS, security checks; opt-in **Backward Compatibility Audit** for API, REST, DTO, config, crypto, and JPA contract changes
4. **Token Optimization** - 50-60% reduction in AI token usage
5. **Pattern-Based Generation** - Consistent, repeatable outputs
6. **Multi-Stack Support** - Java/Spring Boot, Node.js, Python, Go, and generic stacks
...
### Step 2 — Generate Repo Config (first time in each repo)

```bash
gh copilot agent repo-config
```

The agent reads your project files, detects the stack, confirms values, and writes `.github/copilot-config.yml`.
This file is **gitignored** — each developer runs this once.
...
- Resolve config: check `.github/copilot-config.yml` (local) first, fall back to `~/.copilot/config/copilot-config.yml`
- Load test patterns from resolved patterns path
...
- Resolve config: check `.github/copilot-config.yml` (local) first, fall back to `~/.copilot/config/copilot-config.yml`
- Load review patterns from resolved patterns path
...
Agents check for config in this order:
1. `.github/copilot-config.yml` — repo-local, gitignored, generated by `repo-config` agent
2. `~/.copilot/config/copilot-config.yml` — global default (from installer)
3. If neither: prompt user to run `gh copilot agent repo-config`

### Generating Per-Repo Config

```bash
gh copilot agent repo-config
```

Auto-detects: Java/Maven, Java/Gradle, Node.js, Python, Go. Writes `.github/copilot-config.yml` and updates `.gitignore`.

### Project Configuration (`config/copilot-config.yml`) — this source repo

Key settings for this repo:
...
- [ ] All agent, skill, and template files present (including `repo-config.agent.md`)
- [ ] Backward compatibility skill present (`skills/backward-compat/backward-compat.skill.md`)
- [ ] Pattern files synced and identical
- [ ] Sync script executable (`chmod +x .gitlab/sync-patterns.sh`)
- [ ] Run `gh copilot agent repo-config` in your target repo to generate local config
