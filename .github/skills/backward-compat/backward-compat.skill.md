...
description: Checks Java and Spring Boot code changes for backward compatibility issues — public API breaks, REST contract changes, DTO/serialization breaks, config property renames, Spring bean contract changes, JPA entity field removals, and encryption/decryption algorithm changes.
---
...
5. **STRICT SCOPE** — Review ONLY files provided; never expand to related files unless explicitly requested
...
This restriction is absolute. No other file types are ever eligible.
...
```text
The following file(s) are not eligible for backward compatibility review and have been excluded:
...
If the eligible set is **empty** → stop. Display the **No Files Found** message.
...
Collect **all** entries including staged, unstaged, and untracked (`??`) files.

Filter to `*.java`, `*.properties`, `*.yml`, `*.yaml` only.

If the filtered list is **not empty** → review only those files.

If the filtered list is **empty** → stop. Display the **No Files Found** message.

---

# No Files Found

When triggered, **stop immediately**. Do not generate a report.

```text
No eligible files were found for backward compatibility review.
...
Please provide at least one file of a supported type to begin the review, for example:
  ...
  "Check backward compatibility of src/main/java/com/example/service/UserService.java"
  "Are my changes backward compatible?"
```

---

# Backward Compatibility Check Categories

Evaluate each in-scope file for the breaking changes below.

All findings are **HIGH** severity unless explicitly stated otherwise.

---

## 1. Public Java API

Changes that break existing callers of your public API.

- **Removed `public`/`protected` method** — any caller that invokes the removed method breaks at compile-time or runtime
- **Renamed `public`/`protected` method** — same effect as removal; old name no longer resolves
- **Changed method signature** — added, removed, or reordered parameters; changed parameter types
- **Changed return type** — callers expecting the old type break at compile-time
- **Narrowed access modifier** — `public` → `protected`/`package-private`/`private` breaks callers outside the package
- **Removed `public`/`protected` class, interface, or enum** — all references break
- **Renamed `public`/`protected` class, interface, or enum** — all import statements and type references break
- **Removed field from `public` class** — direct field access breaks; also breaks serialization (see DTO section)
- **Changed constructor signature** — callers using `new MyClass(...)` with the old signature break
...
## 3. DTO and Serialization Contracts
...
- **Changed collection element type** — e.g. `List<String>` → `List<Long>`
...
- **Removed profile-specific property file** — deployments using that profile lose their override

---

## 5. Spring Bean Contracts
...
Always produce the report with the following structure.

## ⚠️ Backward Compatibility Warnings

> This section is **only present** when breaking changes are detected.
> If no breaking changes are found, display the **All Clear** message instead.
...
```
Mode: Backward Compatibility Audit
Files Reviewed:
  1. <file path>
  2. ...

Files Excluded:
  - <rejected files, if any>
```
...
Category      : <e.g. Public Java API / Encryption — Cipher Mode / REST Contract>
File          : <file path>
Line          : <line number or range, if identifiable>
Issue         : <one-sentence description of the breaking change>
Impact        : <who or what breaks and how — callers, clients, stored data, etc.>
Recommendation: <actionable fix — versioned method, migration, dual-read path, etc.>
─────────────────────────────────────────────────────────────
```
...
If **no breaking changes** are found, display:

```
✅ No backward compatibility issues detected.

All public APIs, REST contracts, DTO fields, configuration properties,
Spring bean contracts, JPA mappings, and cryptographic configurations
appear unchanged or remain compatible with existing consumers.
```
...
1. Be precise — report only evidence-based findings from the files in scope.
...
Act as an experienced Principal Java Architect performing a backward compatibility audit focused on:

- Public Java API stability (methods, classes, constructors, fields)
- REST endpoint and parameter contracts
- DTO and JSON serialization contracts
- Spring configuration property contracts
- Spring bean injection contracts
- JPA/persistence schema contracts
