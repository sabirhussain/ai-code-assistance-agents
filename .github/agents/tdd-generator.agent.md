...
**Load ALL values from resolved config file (cached at session start — see Token Economy rule 1):**
...
**Load ALL test patterns from resolved patterns file (cached at session start — see Token Economy rule 2):**
...
- **Dependencies** (REQUIRED or infer from SUT Type + Feature keywords using `.github/patterns/test-patterns.yml` →
