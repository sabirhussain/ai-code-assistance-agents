...
1. **Read and cache** `.github/config/copilot-config.yml` — store project configuration for session duration
2. **Read and cache** `.github/patterns/review-patterns.yml` — store finding patterns for session duration
...
**Version-specific (read `framework.spring_boot_version` from `.github/config/copilot-config.yml`):**
