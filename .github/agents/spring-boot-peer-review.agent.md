...
# Optional Skill — Backward Compatibility Audit

**This skill is opt-in and only invoked when the user explicitly requests it.**

Invoke the `backward-compat` skill when the user's request contains any of the following trigger phrases:
...
See `.github/skills/backward-compat/backward-compat.skill.md` for complete specification.

The `backward-compat` skill checks for breaking changes in:

- Public Java API (methods, classes, constructors, fields)
- REST endpoint and parameter contracts
- DTO and JSON serialization contracts
- Spring configuration property contracts
- Spring bean injection contracts (`@Qualifier`, `@Bean` names)
...
- Empty or swallowed catch blocks (silent failures)
- Overly broad catch (`Exception`, `Throwable`) without deliberate justification
- Checked exceptions in Spring components (`@Service`, `@Component`) — breaks `@Transactional` rollback by default
- Missing `@ControllerAdvice` + `@ExceptionHandler` for centralised error handling
- Raw exception detail or stack trace returned to API clients
- Exception not passed to `log.error(msg, e)` — stack trace lost
- `e.printStackTrace()` — bypasses logging framework

**Version-specific (read `framework.spring_boot_version` from resolved config — see Token Economy rule 1):**

- Spring Boot 3.x: `ProblemDetail` (RFC 9457) not used in `@ControllerAdvice` (LOW)
- Spring Boot 2.x: `ResponseEntityExceptionHandler` not used as `@ControllerAdvice` base

---

## Priority 4 - Spring Boot Non-Negotiables

These rules apply regardless of Spring Boot or Java version unless stated.

Examples:

- `@Transactional` on controller or repository layer instead of service layer (HIGH)
- `spring.jpa.open-in-view: false` not set — OSIV keeps DB connections open through serialisation (MEDIUM)
- 3+ related `@Value` annotations — should be a `@ConfigurationProperties` class (MEDIUM)
- `System.out.println` / `System.err` instead of SLF4J + `@Slf4j` (MEDIUM)
- Actuator endpoints exposed with wildcard `*` — security risk in non-local environments (HIGH)
- `@RequestBody` without `@Valid`/`@Validated` — invalid input reaches service layer (MEDIUM)
- `null` returned from `@RequestMapping` method — ambiguous HTTP 200; use `ResponseEntity` (MEDIUM)
- No profile-based configuration — infrastructure config hardcoded without profile overrides (MEDIUM)
...
- Hidden dependencies (direct `new` in business logic)
...
**Test type guidance (use the right slice):**

- `@WebMvcTest` — controller layer only (no full context)
- `@DataJpaTest` — repository layer only (in-memory DB, no full context)
- Plain JUnit + Mockito — service and domain logic (no Spring context)
- `@SpringBootTest` — full integration tests only; flag if used where a slice would suffice
...
## Priority 7 - Modernization

Examples:

- JDK improvements (version-aware: JDK 8+ through JDK 21+)
...
## Priority 8 - Backward Compatibility *(opt-in — invoke `backward-compat` skill when explicitly requested)*

Examples:

- Removed or renamed `public`/`protected` methods, classes, constructors, fields
- Changed method signatures or return types
- Removed or renamed REST endpoints or changed their HTTP method
- Added required request parameters without a default value
- Removed or renamed request/response DTO fields (`@JsonProperty`, field names)
- Removed or renamed Spring config properties
- Removed or renamed `@Bean`/`@Qualifier` names
- Removed JPA entity fields mapped to database columns
- Changed checked exception types on `public` methods *(MEDIUM)*
- Changed encryption algorithm, key size, cipher mode, or padding scheme without migration path
- Changed IV handling, ciphertext encoding, password hashing algorithm, or salt strategy
- Changed KDF parameters (PBKDF2 iteration count, PRF, key length)
